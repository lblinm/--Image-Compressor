...
double compressDiagonalInfo[6];
Mat compressImgDiagonal(Mat& img,int xigema)//分别输入1.原灰度图 2.转灰度图文件名 3.解码后的文件名 4.EPSILON
{
        ...
        compressDiagonalInfo[0]=timer.elapsed();
        ...
        compressDiagonalInfo[1]=timer.elapsed()-int(compressDiagonalInfo[0]);


        compressDiagonalInfo[2]=PSNR(img_gray,T,M,N);
        compressDiagonalInfo[3]=colorList.size();

        double BPPValue = BPP(colorList,M,N,coordinateList);
        compressDiagonalInfo[4]=BPPValue;
        compressDiagonalInfo[5]=8.0 / BPPValue;
